...
- 2026-05-11: Scroll-Animation-Flicker gefixt. Ursache: `img.src = …` swap triggert Async-Decode → kurzer Bitmap-Gap pro Frame. Lösung: `<img>` durch `<canvas>` ersetzt, alle 50 Frames mit `img.decode()` vorab dekodiert, jeder Scroll-Step rendert synchron via `ctx.drawImage()`. DPR-aware Canvas-Sizing + Resize-Handler. Markup/Effekt-Look identisch.
- 2026-05-11: Scroll-anim Flicker-Fix v2 — drop-shadow vom Canvas auf statisches Pseudo-Element verlagert, Canvas auf opaque (alpha:false) mit BG-Fill statt clearRect, DPR-Cap 1.5
- 2026-05-11: Bilder slide-in von der Seite beim Scrollen. Neue Reveal-Variante `.reveal-stagger.from-side` (odd → von links, even → von rechts, translateX 130px, 1.2s cubic-bezier) angewendet auf cat-grid (Kollektionen), feat-grid (Featured) und detail-grid (Detail). IntersectionObserver-Selector um `.reveal-left/.reveal-right` erweitert. Body hat schon overflow-x:hidden — kein horizontaler Scroll. Reduced-motion respektiert.
- 2026-05-11: Scroll-Agent (GSAP 3.12 + ScrollTrigger + SplitText) zusätzlich drübergelegt, Preset `luxury`. Hero-Parallax, Fade-Up auf Sections/Headlines, Stagger auf Grid-Items, 30 Bilder mit sanftem Y-Reveal annotiert. Injection idempotent zwischen `<!-- SCROLL-AGENT:START/END -->`-Markern, koexistiert sauber neben dem custom IntersectionObserver-Reveal.
